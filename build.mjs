...
if (!html.includes(spriteLink)) html = html.replace('</head>', `  ${spriteLink}\n</head>`);
const spriteDirectionScript = '<script src="/sprite-directions.js" defer></script>';
if (!html.includes(spriteDirectionScript)) html = html.replace('</body>', `  ${spriteDirectionScript}\n</body>`);
...
const probes = [
  ['campStage markup', 'id="campStage"'],
  ['campTerrain markup', 'id="campTerrain"'],
  ['campGround markup', 'id="campGround"'],
  ['campBuilding fn', 'function campBuilding('],
  ['renderCamp fn', 'function renderCamp('],
  ['campStage innerHTML', 'campStage.innerHTML'],
  ['refs campStage', 'campStage:'],
  ['morgueLevel', 'morgueLevel'],
  ['morgueButton', 'morgueButton'],
  ['morgue upgrade ru', 'морг · ур.'],
  ['morgue building call', 'campBuilding("morgue"'],
  ['development fn', 'function campDevelopment('],
  ['development assignment', 'const development ='],
  ['signature', 'const signature = JSON.stringify({']
];
const rows=[];
for (const [label, needle] of probes) {
  const idx=html.toLowerCase().indexOf(needle.toLowerCase());
  if(idx<0){rows.push(`===== ${label}: NOT FOUND =====`);continue;}
  const a=Math.max(0,idx-650), b=Math.min(html.length,idx+needle.length+1500);
